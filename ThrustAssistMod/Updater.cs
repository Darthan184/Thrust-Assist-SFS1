                            ...
                            if (thisTime>lastTime+timeStep)
                            {
                                ...
                                foreach (SFS.Parts.Modules.EngineModule oneEngine in rocket.partHolder.GetModules<SFS.Parts.Modules.EngineModule>())
                                {
                                    if (oneEngine.engineOn.Value)
                                    {
                                        ...
                                        thrustVector += direction*oneEngine.maxThrust.Value;
                                    }
                                }
...
//~                                 double angle =  NormaliseAngle(location.position.AngleDegrees - rocket.GetRotation());
                                ...
                                else
                                {
                                    ...
                                    Double2 forward =  Double2.up.Rotate(0.0 - (this.position.AngleRadians - Math.PI / 2.0));
                                    ...
                                    Double2 velocity = location.velocity.Rotate(0.0 - (location.position.AngleRadians - Math.PI / 2.0));
                                    Double2 targetDistance = new Double2
                                        (
                                            NormaliseAngle(ThrustAssistMod.UI.Marker -location.position.AngleDegrees)*radiansPerDegree*location.Radius
                                            ,targetHeight-height;
                                        );
